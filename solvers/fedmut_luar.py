...
class FedMUT_LUAR:
    ...
    def average_model (self, checkpoint, epoch_id, num_comms):
        ...
        for i in range (len(checkpoint.models[0].trainable_variables)):
            if len(checkpoint.models[0].trainable_variables[i].shape) > 1:
                if i not in self.recycling_layers: # update
                    ...
                    new_param = tf.math.add(update, self.prev_params[i])

                    np_last_t_param = np.array(self.prev_params[i])
                    self.prev_updates[i] = np.subtract(new_param, np_last_t_param)
                    self.score[offset] = np.linalg.norm(self.prev_updates[i].flatten()) / (np.linalg.norm(np_last_t_param.flatten()) + 1e-6)
                    num_comms[i] += 1

                    average_param_store.append(new_param)

                    if len(new_param.shape) > 1:
                        weight_deltas.append(tf.math.subtract(new_param, self.prev_params[i]))
                        global_weights.append(new_param)

                else: # recycle
                    prev_updates_tensor = tf.convert_to_tensor(self.prev_updates[i])
                    new_param = tf.math.add(self.prev_params[i], prev_updates_tensor)
                    average_param_store.append(new_param)
                ...
                globalsum_param = self.comm.allreduce(localsum_param, op=MPI.SUM)
                update = globalsum_param / self.num_workers
                global_param = tf.math.add(update, self.prev_params[i])

                average_param_store.append(global_param)
                if len(global_param.shape) > 1:
                        weight_deltas.append(tf.math.subtract(global_param, self.prev_params[i]))
                        global_weights.append(global_param)
                        
                self.prev_params[i] = global_param
        ...
        for i in range (len(checkpoint.models[0].trainable_variables)):
            for j in range(self.num_local_workers):
                if len(average_param_store[i].shape) > 1:
                    checkpoint.models[j].trainable_variables[i].assign(mutated_model[j][index])
                    flag = True
                else:                    
                    checkpoint.models[j].trainable_variables[i].assign(average_param_store[i])
